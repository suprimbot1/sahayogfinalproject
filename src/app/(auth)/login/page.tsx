...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Trigger standard NextAuth credentials flow
    ...
    if (result?.error) {
      ...
      // Success! NextAuth redirects depending on your role, but we can hard wire the front-end to fast-redirect.
      // Super admin uses "suprim" directly
      if (username === "suprim") {
      ...
      <div className="w-full bg-[#0D0D0F]/90 backdrop-blur-xl border border-white/5 rounded-2xl p-8 shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <label className="text-[13px] font-semibold text-white ml-1">Username or Email address</label>
              <input
                type="text"
                value={username}
          ...
          <div className="space-y-1.5 flex flex-col">
            <div className="flex items-center justify-between ml-1 leading-none">
                ...
                <a href="#" className="text-[13px] text-[#696CFF] hover:text-[#8050FF] font-medium transition-colors">Forgot password?</a>
            </div>
            <input
              type="password"
              value={password}
